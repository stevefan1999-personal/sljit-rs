...
impl Function {
    ...
    fn emit_set_local(
        ...
        match (local, val) {
            ...
            (LocalVar::Stack(offset), StackValue::ConstF64(bits)) => {
                ...
                {
                    emitter.mov(0, mem_sp_offset(*offset), *bits as usize)?;
                }
    ...
    fn compile_operator(
        ...
        match op {
            ...
            Operator::I32Add => self.compile_binary_op(emitter, Op2::Add32),
            Operator::I32Sub => self.compile_binary_op(emitter, Op2::Sub32),
            Operator::I32Mul => self.compile_binary_op(emitter, Op2::Mul32),
            Operator::I32And => self.compile_binary_op(emitter, Op2::And32),
            Operator::I32Or => self.compile_binary_op(emitter, Op2::Or32),
            Operator::I32Xor => self.compile_binary_op(emitter, Op2::Xor32),
            Operator::I32Shl => self.compile_binary_op(emitter, Op2::Shl32),
            Operator::I32ShrS => self.compile_binary_op(emitter, Op2::Ashr32),
            Operator::I32ShrU => self.compile_binary_op(emitter, Op2::Lshr32),
            Operator::I32Rotl => self.compile_binary_op(emitter, Op2::Rotl32),
            Operator::I32Rotr => self.compile_binary_op(emitter, Op2::Rotr32),
            ...
            // Unary operations
            Operator::I32Clz => self.compile_unary_op(emitter, Op1::Clz32),
            Operator::I32Ctz => self.compile_unary_op(emitter, Op1::Ctz32),
            Operator::I32Popcnt => self.compile_unary_op_with_helper(emitter, UnaryOp::Popcnt),

            // Comparison operations
            Operator::I32Eqz => {
                let val = self.pop_value()?;
                let reg = self.ensure_in_register(emitter, val)?;
                emitter.emit_op2u((Op2::Xor32, sys::SLJIT_SET_Z), reg, 0i32)?;
                emitter.op_flags(sys::SLJIT_MOV32, reg, Condition::Equal as i32)?;
            ...
            Operator::Select => {
                ...
                let reg1 = self.ensure_in_register(emitter, val1)?;

                emitter.emit_op2u((Op2::Xor, sys::SLJIT_SET_Z), cond.to_operand(), 0i32)?;
                emitter.select(Condition::Equal, reg1, val2.to_operand(), reg1)?;
            ...
            Operator::I32Load { memarg } => {
                self.compile_load_op(emitter, memarg.offset as i32, Op1::Mov32)
            }
            Operator::I32Load8S { memarg } => {
                self.compile_load_op(emitter, memarg.offset as i32, Op1::MovS8)
            }
            Operator::I32Load8U { memarg } => {
                self.compile_load_op(emitter, memarg.offset as i32, Op1::MovU8)
            }
            Operator::I32Load16S { memarg } => {
                self.compile_load_op(emitter, memarg.offset as i32, Op1::MovS16)
            }
            Operator::I32Load16U { memarg } => {
                self.compile_load_op(emitter, memarg.offset as i32, Op1::MovU16)
            }
            Operator::I32Store { memarg } => {
                self.compile_store_op(emitter, memarg.offset as i32, Op1::Mov32)
            }
            Operator::I32Store8 { memarg } => {
                self.compile_store_op(emitter, memarg.offset as i32, Op1::MovU8)
            }
            Operator::I32Store16 { memarg } => {
                self.compile_store_op(emitter, memarg.offset as i32, Op1::MovU16)
            }

            #[cfg(target_pointer_width = "64")]
            Operator::I64Const { value } => {
                let reg = self.alloc_register(emitter)?;
                emitter.mov(0, reg, *value as usize)?;
                self.stack.push(StackValue::Register(reg));
                Ok(())
            }
            ...
            Operator::I64Add => self.compile_binary_op64(emitter, Op2::Add),
            Operator::I64Sub => self.compile_binary_op64(emitter, Op2::Sub),
            Operator::I64Mul => self.compile_binary_op64(emitter, Op2::Mul),
            Operator::I64And => self.compile_binary_op64(emitter, Op2::And),
            Operator::I64Or => self.compile_binary_op64(emitter, Op2::Or),
            Operator::I64Xor => self.compile_binary_op64(emitter, Op2::Xor),
            Operator::I64Shl => self.compile_binary_op64(emitter, Op2::Shl),
            Operator::I64ShrS => self.compile_binary_op64(emitter, Op2::Ashr),
            Operator::I64ShrU => self.compile_binary_op64(emitter, Op2::Lshr),
            Operator::I64Rotl => self.compile_binary_op64(emitter, Op2::Rotl),
            Operator::I64Rotr => self.compile_binary_op64(emitter, Op2::Rotr),
            ...
            Operator::I64Clz => self.compile_unary_op_with_helper(emitter, UnaryOp::Clz64),
            Operator::I64Ctz => self.compile_unary_op_with_helper(emitter, UnaryOp::Ctz64),
            Operator::I64Popcnt => self.compile_unary_op_with_helper(emitter, UnaryOp::Popcnt64),

            // I64 comparison operations
            Operator::I64Eqz => {
                let val = self.pop_value()?;
                let reg = self.ensure_in_register(emitter, val)?;
                emitter.emit_op2u((Op2::Xor, sys::SLJIT_SET_Z), reg, 0i32)?;
                emitter.op_flags(sys::SLJIT_MOV, reg, Condition::Equal as i32)?;
                self.stack.push(StackValue::Register(reg));
                Ok(())
            }
            ...
            Operator::I64Load { memarg } => {
                self.compile_load_op64(emitter, memarg.offset as i32, Op1::Mov)
            }
            Operator::I64Load8S { memarg } => {
                self.compile_load_op64(emitter, memarg.offset as i32, Op1::MovS8)
            }
            Operator::I64Load8U { memarg } => {
                self.compile_load_op64(emitter, memarg.offset as i32, Op1::MovU8)
            }
            Operator::I64Load16S { memarg } => {
                self.compile_load_op64(emitter, memarg.offset as i32, Op1::MovS16)
            }
            Operator::I64Load16U { memarg } => {
                self.compile_load_op64(emitter, memarg.offset as i32, Op1::MovU16)
            }
            Operator::I64Load32S { memarg } => {
                self.compile_load_op64(emitter, memarg.offset as i32, Op1::MovS32)
            }
            Operator::I64Load32U { memarg } => {
                self.compile_load_op64(emitter, memarg.offset as i32, Op1::MovU32)
            }
            Operator::I64Store { memarg } => {
                self.compile_store_op64(emitter, memarg.offset as i32, Op1::Mov)
            }
            Operator::I64Store8 { memarg } => {
                self.compile_store_op64(emitter, memarg.offset as i32, Op1::MovU8)
            }
            Operator::I64Store16 { memarg } => {
                self.compile_store_op64(emitter, memarg.offset as i32, Op1::MovU16)
            }
            Operator::I64Store32 { memarg } => {
                self.compile_store_op64(emitter, memarg.offset as i32, Op1::Mov32)
            }

            Operator::I32WrapI64 => {
                let val = self.pop_value()?;
                emitter.mov32(0, val.to_operand(), val.to_operand())?;
                self.stack.push(val);
                Ok(())
            }
            #[cfg(target_pointer_width = "64")]
            ...
            Operator::F32Load { memarg } => {
                self.compile_float_load_op(emitter, memarg.offset as i32, Fop1::MovF32)
            }
            Operator::F64Load { memarg } => {
                self.compile_float_load_op(emitter, memarg.offset as i32, Fop1::MovF64)
            }
            Operator::F32Store { memarg } => {
                self.compile_float_store_op(emitter, memarg.offset as i32, Fop1::MovF32)
            }
            Operator::F64Store { memarg } => {
                self.compile_float_store_op(emitter, memarg.offset as i32, Fop1::MovF64)
            }

            // F32 binary operations
            Operator::F32Add => self.compile_float_binary_op(emitter, Fop2::AddF32, true),
            Operator::F32Sub => self.compile_float_binary_op(emitter, Fop2::SubF32, true),
            Operator::F32Mul => self.compile_float_binary_op(emitter, Fop2::MulF32, true),
            Operator::F32Div => self.compile_float_binary_op(emitter, Fop2::DivF32, true),
            Operator::F32Min => {
                self.compile_float_binary_op_with_helper(emitter, FloatBinaryOp::Min, true)
            }
            Operator::F32Max => {
                self.compile_float_binary_op_with_helper(emitter, FloatBinaryOp::Max, true)
            }
            Operator::F32Copysign => {
                self.compile_float_binary_op_with_helper(emitter, FloatBinaryOp::Copysign, true)
            }

            // F64 binary operations
            Operator::F64Add => self.compile_float_binary_op(emitter, Fop2::AddF64, false),
            Operator::F64Sub => self.compile_float_binary_op(emitter, Fop2::SubF64, false),
            Operator::F64Mul => self.compile_float_binary_op(emitter, Fop2::MulF64, false),
            Operator::F64Div => self.compile_float_binary_op(emitter, Fop2::DivF64, false),
            Operator::F64Min => {
                self.compile_float_binary_op_with_helper(emitter, FloatBinaryOp::Min, false)
            }
            Operator::F64Max => {
                self.compile_float_binary_op_with_helper(emitter, FloatBinaryOp::Max, false)
            }
            Operator::F64Copysign => {
                self.compile_float_binary_op_with_helper(emitter, FloatBinaryOp::Copysign, false)
            }

            // F32 unary operations
            Operator::F32Neg => self.compile_float_unary_op(emitter, Fop1::NegF32, true),
            Operator::F32Abs => self.compile_float_unary_op(emitter, Fop1::AbsF32, true),
            Operator::F32Sqrt => {
                self.compile_float_unary_op_with_helper(emitter, FloatUnaryOp::Sqrt, true)
            }
            Operator::F32Ceil => {
                self.compile_float_unary_op_with_helper(emitter, FloatUnaryOp::Ceil, true)
            }
            Operator::F32Floor => {
                self.compile_float_unary_op_with_helper(emitter, FloatUnaryOp::Floor, true)
            }
            Operator::F32Trunc => {
                self.compile_float_unary_op_with_helper(emitter, FloatUnaryOp::Trunc, true)
            }
            Operator::F32Nearest => {
                self.compile_float_unary_op_with_helper(emitter, FloatUnaryOp::Nearest, true)
            }

            // F64 unary operations
            Operator::F64Neg => self.compile_float_unary_op(emitter, Fop1::NegF64, false),
            Operator::F64Abs => self.compile_float_unary_op(emitter, Fop1::AbsF64, false),
            Operator::F64Sqrt => {
                self.compile_float_unary_op_with_helper(emitter, FloatUnaryOp::Sqrt, false)
            }
            Operator::F64Ceil => {
                self.compile_float_unary_op_with_helper(emitter, FloatUnaryOp::Ceil, false)
            }
            Operator::F64Floor => {
                self.compile_float_unary_op_with_helper(emitter, FloatUnaryOp::Floor, false)
            }
            Operator::F64Trunc => {
                self.compile_float_unary_op_with_helper(emitter, FloatUnaryOp::Trunc, false)
            }
            Operator::F64Nearest => {
                self.compile_float_unary_op_with_helper(emitter, FloatUnaryOp::Nearest, false)
            }
            ...
            Operator::F32ReinterpretI32 => {
                ...
                let temp_offset = (self.frame_offset + 7) & !7;
                self.frame_offset = temp_offset + 8;
                emitter.mov32(0, mem_sp_offset(temp_offset), val.to_operand())?;
                emitter.mov_f32(SLJIT_32, freg, mem_sp_offset(temp_offset))?;
            ...
            Operator::F64ReinterpretI64 => {
                ...
                #[cfg(target_pointer_width = "64")]
                {
                    emitter.mov(0, mem_sp_offset(temp_offset), val.to_operand())?;
                ...
                #[cfg(not(target_pointer_width = "64"))]
                {
                    ...
                    match val {
                        StackValue::Stack(value_offset) => {
                            emitter.mov_f64(0, freg, mem_sp_offset(value_offset))?;
                        }
                        _ => {
                            return Err(CompileError::Invalid(
    ...
    fn compile_compare_op(
        ...
        emitter.emit_op2u((Op2::Sub32, flags), reg_a, b.to_operand())?;

        // Convert flags to 0/1 using op_flags - use MOV32 for 32-bit result
        emitter.op_flags(sys::SLJIT_MOV32, reg_a, (cond as i32) & !SLJIT_32)?;
    ...
    fn compile_unary_op(&mut self, emitter: &mut Emitter, op: Op1) -> Result<(), CompileError> {
        let val = self.pop_value()?;
        let reg = self.ensure_in_register(emitter, val)?;
        emitter.emit_op1((op, 0), reg, reg)?;
        self.stack.push(StackValue::Register(reg));
        Ok(())
    }

    fn compile_unary_op_with_helper(
        &mut self,
        emitter: &mut Emitter,
        op: UnaryOp,
    ) -> Result<(), CompileError> {
        ...
        let mut helper = |func_addr: usize| {
            ...
            let offset_a = self.ensure_i64_on_stack(emitter, val)?;
            ...
            emitter.get_local_base(ScratchRegister::R0, offset_a)?;
            emitter.get_local_base(ScratchRegister::R1, result_offset)?;

            // arg_types for (W, W) -> void (2 pointer arguments, void return)
            emitter.icall(JumpType::Call as i32, arg_types!([W, W]), func_addr)?;
        ...
        match op {
            UnaryOp::Popcnt => {
                let val = self.pop_value()?;
                let reg = self.ensure_in_register(emitter, val)?;
                emitter.mov(0, ScratchRegister::R0, reg)?;
                emitter.icall(
                    JumpType::Call as i32,
                    arg_types!([W] -> W),
                    helpers::popcnt32 as *const () as usize,
                )?;
                emitter.mov(0, reg, ScratchRegister::R0)?;
                self.stack.push(StackValue::Register(reg));
            }
            ...
            #[cfg(target_pointer_width = "64")]
            UnaryOp::Clz64 => self.compile_unary_op(emitter, Op1::Clz)?,
            #[cfg(target_pointer_width = "64")]
            UnaryOp::Ctz64 => self.compile_unary_op(emitter, Op1::Ctz)?,
    ...
    fn compile_load_op(
        ...
        emitter.emit_op1((kind, 0), addr_reg, mem_offset(addr_reg, offset))?;
        self.stack.push(StackValue::Register(addr_reg));
        Ok(())
    }

    fn compile_float_load_op(
        &mut self,
        emitter: &mut Emitter,
        offset: i32,
        kind: Fop1,
        ...
        emitter.add(0, addr_reg, addr_reg, SavedRegister::S0)?;
        let freg = self.alloc_float_register(emitter)?;
        emitter.emit_fop1((kind, 0), freg, mem_offset(addr_reg, offset))?;
    ...
    fn compile_store_op(
        ...
        emitter.emit_op1((kind, 0), mem_offset(addr_reg, offset), value_reg)?;
        self.occupied_registers.set(value_reg.index(), false);
        self.occupied_registers.set(addr_reg.index(), false);
        Ok(())
    }

    fn compile_float_store_op(
        &mut self,
        emitter: &mut Emitter,
        offset: i32,
        kind: Fop1,
        ...
        emitter.emit_fop1((kind, 0), mem_offset(addr_reg, offset), freg)?;
        self.occupied_float_registers.set(freg.index(), false);
        self.occupied_registers.set(addr_reg.index(), false);
        Ok(())
    }
    ...
    #[cfg(target_pointer_width = "64")]
    fn compile_binary_op64(&mut self, emitter: &mut Emitter, op: Op2) -> Result<(), CompileError> {
        let (b, a) = (self.pop_value()?, self.pop_value()?);
        let reg_a = self.ensure_in_register(emitter, a)?;
        let operand_b = b.to_operand();
        emitter.emit_op2((op, 0), reg_a, reg_a, operand_b)?;
    ...
    fn compile_binary_op64(&mut self, emitter: &mut Emitter, op: Op2) -> Result<(), CompileError> {
        ...
        let offset_a = self.ensure_i64_on_stack(emitter, a)?;
        let offset_b = self.ensure_i64_on_stack(emitter, b)?;
        ...
        emitter.get_local_base(ScratchRegister::R0, offset_a)?;
        emitter.get_local_base(ScratchRegister::R1, offset_b)?;
        emitter.get_local_base(ScratchRegister::R2, result_offset)?;
    ...
    fn ensure_i64_on_stack(
        ...
        match val {
            ...
            StackValue::Const(v) => {
                ...
                let offset = self.frame_offset;
                self.frame_offset += 8;
                emitter.mov32(0, mem_sp_offset(offset), v)?;
                emitter.mov32(0, mem_sp_offset(offset + 4), 0i32)?;
                Ok(offset)
            }
            StackValue::Register(reg) => {
            ...
            StackValue::Saved(sreg) => {
                ...
                let offset = self.frame_offset;
                self.frame_offset += 8;
                emitter.mov32(0, mem_sp_offset(offset), sreg)?;
                emitter.mov32(0, mem_sp_offset(offset + 4), 0i32)?;
                Ok(offset)
            }
            _ => Err(CompileError::Invalid(
    ...
    fn compile_compare_op64(
        ...
        self.compile_compare_op(emitter, cond)
    }
    ...
    fn compile_load_op64(
        ...
        match kind {
            ...
            Op1::MovS8 => {
                ...
                emitter.mov32(0, mem_sp_offset(result_offset), addr_reg)?;
                // Sign extend to high part
                emitter.ashr32(0, addr_reg, addr_reg, 31i32)?;
                emitter.mov32(0, mem_sp_offset(result_offset + 4), addr_reg)?;
            }
            Op1::MovU8 => {
                emitter.mov_u8(
                    0,
    ...
    fn compile_store_op64(
        ...
        let value_reg = match &value {
            StackValue::Stack(value_offset) => {
                let reg = self.alloc_register(emitter)?;
                emitter.mov32(0, reg, mem_sp_offset(*value_offset))?;
                reg
            }
            _ => self.ensure_in_register(emitter, value.clone())?,
        };
        ...
        match kind {
            Op1::Mov => {
                ...
                if let StackValue::Stack(value_offset) = &value {
                    emitter.mov32(0, value_reg, mem_sp_offset(*value_offset + 4))?;
                    emitter.mov32(0, mem_offset(addr_reg, offset + 4), value_reg)?;
                } else {
                    self.occupied_registers.set(value_reg.index(), false);
                    self.occupied_registers.set(addr_reg.index(), false);
                    return Err(CompileError::Invalid(
            ...
            Op1::MovU8 | Op1::MovU16 | Op1::Mov32 => {
                emitter.emit_op1((kind, 0), mem, value_reg)?;
            }
            _ => unreachable!(),
        }
    ...
    fn compile_float_binary_op(
        ...
        emitter.emit_fop2((op, 0), freg_a, freg_a, freg_b)?;
    ...
    fn compile_float_unary_op(
        ...
        emitter.emit_fop1((op, 0), freg, freg)?;
        self.stack.push(StackValue::FloatRegister(freg));
        Ok(())
    }
    ...
    fn compile_float_compare_op(
        ...
        let (b, a) = (self.pop_value()?, self.pop_value()?);
        let reg_a = self.ensure_in_float_register(emitter, a, is_f32)?;
        let reg_b = self.ensure_in_float_register(emitter, b, is_f32)?;

        // Map condition to the appropriate SET flag
        if is_f32 {
            emitter.cmp_f32(flags, reg_a, reg_b)?;
        } else {
            emitter.cmp_f64(flags, reg_a, reg_b)?;
        }

        // Convert flags to 0/1 using op_flags
        emitter.op_flags(sys::SLJIT_MOV, reg_a, cond as i32)?;
    ...
    fn compile_convert_int_to_float(
        ...
        match (is_f32, is_i32, is_signed) {
            (true, true, true) => emitter.conv_f32_from_s32(0, freg, reg)?,
            (true, true, false) => emitter.conv_f32_from_u32(0, freg, reg)?,
            (false, true, true) => emitter.conv_f64_from_s32(0, freg, reg)?,
            (false, true, false) => emitter.conv_f64_from_u32(0, freg, reg)?,
            (true, false, true) => emitter.conv_f32_from_sw(0, freg, reg)?,
            (true, false, false) => emitter.conv_f32_from_uw(0, freg, reg)?,
            (false, false, true) => emitter.conv_f64_from_sw(0, freg, reg)?,
            (false, false, false) => emitter.conv_f64_from_uw(0, freg, reg)?,
        };
    ...
    fn compile_convert_int_to_float(
        ...
        if !is_i32 {
            ...
            emitter.mov32(0, ScratchRegister::R0, mem_sp_offset(value_offset))?;
            emitter.mov32(0, ScratchRegister::R1, mem_sp_offset(value_offset + 4))?;
        ...
        match (is_f32, is_signed) {
            (true, true) => emitter.conv_f32_from_s32(0, freg, reg)?,
            (true, false) => emitter.conv_f32_from_u32(0, freg, reg)?,
            (false, true) => emitter.conv_f64_from_s32(0, freg, reg)?,
            (false, false) => emitter.conv_f64_from_u32(0, freg, reg)?,
        };
    ...
    fn compile_convert_float_to_int(
        ...
        match (is_f32, is_i32) {
            (true, true) => emitter.conv_s32_from_f32(0, reg, freg)?,
            (false, true) => emitter.conv_s32_from_f64(0, reg, freg)?,

            (true, false) => emitter.conv_sw_from_f32(0, reg, freg)?,

            (false, false) => emitter.conv_sw_from_f64(0, reg, freg)?,
        };
    ...
    fn compile_global_set(
        ...
        emitter.mov(0, addr_reg, global.ptr)?;
    ...
    fn compile_memory_grow(
        ...
        if delta_reg != ScratchRegister::R0 {
            emitter.mov(0, ScratchRegister::R0, size_ptr)?;
        ...
        let addr_reg = self.alloc_register(emitter)?;
        emitter.mov(0, addr_reg, grow_callback as usize)?;
    ...
    fn compile_call_indirect(
        ...
        emitter.mov(0, func_ptr_reg, table_data_ptr)?;
